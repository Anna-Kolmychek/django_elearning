...
class LessonsTestCase(test.APITestCase):
    ...
    def create_users(self):
        ...
        self.user2 = User.objects.create(email='user2@email.com', password='123')
    ...
    def create_moderator(self):
        ...
        self.moderator = User.objects.create(email='moderator@email.com', password='123')
    ...
    def test_create_lesson(self):
        ...
        # Case 1. Не авторизованный пользователь не может создать урок
        ...
        # Case 2. Авторизованный пользователь не может создать урок, записываются верные данные (с владельцем)
        ...
        self.assertEquals(
            response.json(),
            {'id': 1, 'title': 'test lesson 1', 'description': 'test description', 'preview': None,
             'video_url': None, 'course': None, 'owner': self.user1.pk}
        )

        # Case 3. Модератор не может создать урок
    ...
    def test_list_lessons(self):
        ...
        # Case 1. Нет доступа у неавторизованного пользователя
        ...
        # Case 2. У пользователя user1 есть его урок, и урок отображается
        self.client.force_authenticate(user=self.user1)
        response = self.client.get(url)
        ...
        # Case 3. У пользователя user2 нет уроков, список пуст
        ...
        # Case 4. У модератора есть доступ, созданный урок отображается
        self.client.force_authenticate(user=self.moderator)
        response = self.client.get(url)
    ...
    def test_retrieve_lessons(self):
        ...
        # Case 2. У владельца есть доступ, данные корректно отображаются
        self.client.force_authenticate(user=self.user1)
        response = self.client.get(url)
        ...
        # Case 3. У авторизованного пользователя не владельца нет доступа
        self.client.force_authenticate(user=self.user2)
        response = self.client.get(url)
        ...
        # Case 4. У модератора есть доступ, данные корректно отображаются
        self.client.force_authenticate(user=self.moderator)
        response = self.client.get(url)
    ...
    def test_update_lessons(self):
        ...
        # Case 2. У владельца есть доступ, данные обновляются
        self.client.force_authenticate(user=self.user1)
        response = self.client.put(url, data=data_user)
        ...
        # Case 4. У модератора есть доступ, данные обновляются
        self.client.force_authenticate(user=self.moderator)
    ...
    def test_destroy_lessons(self):
        ...
        # Case 2. У авторизованного пользователя не владельца нет доступа
        self.client.force_authenticate(user=self.user2)
        response = self.client.delete(url)
        ...
        # Case 3. У модератора нет доступа
        self.client.force_authenticate(user=self.moderator)
        response = self.client.delete(url)
        ...
        # Case 4. У владельца есть доступ, данные удаляются
        self.client.force_authenticate(user=self.user1)
        response = self.client.delete(url)
